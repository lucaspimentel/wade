...
- **File finder** — press `Ctrl+F` to recursively search for files and directories by name in the current directory tree; type to filter, Up/Down to navigate, Enter to open; results prioritize the current directory (breadth-first). Automatically skips `.git` directories and respects hidden/system file visibility settings
