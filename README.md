...
- **Cloud file handling** — OneDrive, Dropbox, and other cloud-synced placeholder files are detected automatically (Windows only); retains the original file-type icon with a cloud status icon in the status column; dim styling; metadata pane shows filename, size, modified date, and "Cloud: not downloaded"; preview pane shows "[cloud file – not downloaded]" instead of attempting to read the file; properties overlay shows "Cloud file" in the Attributes field
