...
- **File metadata** — structured metadata displayed as a header above the preview pane and in the properties overlay (`i` key): NuGet packages (`.nupkg`, `.snupkg`) show Id, Version, Authors, Description, Tags, License, and Dependencies from the embedded `.nuspec`; executables (`.exe`, `.dll`) show PE metadata (architecture, subsystem, timestamp), .NET assembly info (name, version, target framework, referenced assemblies), and Win32 version info on Windows (product, company, copyright); Office documents (`.docx`, `.xlsx`, `.pptx`) show document metadata (title, author, dates, keywords) and statistics (page/word/slide counts, sheet names); PDF files show title, author, subject, creator, and page count (requires `pdfinfo` from xpdf on PATH); media files (`.mp3`, `.mp4`, `.mkv`, `.flac`, etc.) show format, duration, codec, resolution, bitrate, and audio details (requires `ffprobe` or `mediainfo` on PATH)
...
- **Cloud file handling** — OneDrive, Dropbox, and other cloud-synced placeholder files are detected automatically (Windows only); retains the original file-type icon with a cloud status icon in the status column; dim styling; preview pane shows "[cloud file – not downloaded]" instead of attempting to read the file; properties overlay shows "Cloud file" in the Attributes field
