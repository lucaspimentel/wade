...
## Unix/WSL raw terminal mode

Arrow keys (and other special keys) don't work on Unix/WSL because the terminal is never put into raw mode.

**Root cause:** `TerminalSetup` (`src/Wade/Terminal/TerminalSetup.cs`) only configures raw input on Windows via `SetConsoleMode` (disabling line input, echo, processed input). On Unix/WSL, there is no equivalent termios setup — the terminal stays in canonical (cooked) mode where input is line-buffered. Arrow keys emit VT escape sequences (`ESC[A`, etc.) but they're held in the line buffer until Enter is pressed, so `UnixInputSource` never receives them in real time.

**Fix:** Add termios raw-mode setup on Unix in `TerminalSetup`:

1. On construction (Unix path), call `tcgetattr` to save the original termios struct, then apply raw-mode flags:
   - Disable `ICANON` (canonical/line-buffered mode)
   - Disable `ECHO` (character echo)
   - Disable `ISIG` (signal generation for Ctrl+C/Ctrl+Z)
   - Set `VMIN=1`, `VTIME=0` (block until at least 1 byte available)
2. On `Dispose` (Unix path), restore the saved original termios via `tcsetattr`.
3. Implementation options:
   - **P/Invoke `tcgetattr`/`tcsetattr`** from libc — NativeAOT-safe, no child process overhead
   - **Shell out to `stty raw -echo`** — simpler but spawns a process and less reliable for cleanup

**Files to modify:**
- `src/Wade/Terminal/TerminalSetup.cs` — add Unix raw mode setup/teardown
