...
public class FileIconsTests
{
    ...
    [Fact]
    public void Directory_ReturnsFolderIcon() => Assert.Equal(new Rune(0xF114), FileIcons.GetIcon(Dir("src")));

    [Fact]
    public void Drive_ReturnsDriveIcon() => Assert.Equal(new Rune(0xF0A0), FileIcons.GetIcon(Drive("C:")));
    ...
    public void UnknownExtension_ReturnsFallbackFileIcon(string filename) => Assert.Equal(new Rune(0xF15B), FileIcons.GetIcon(File(filename)));

    [Theory]
    [InlineData("Dockerfile")]
    [InlineData("dockerfile")]
    public void Dockerfile_WithoutExtension_ReturnsDockerIcon(string filename) => Assert.Equal(new Rune(0xF308), FileIcons.GetIcon(File(filename)));

    [Theory]
    [InlineData(".gitignore")]
    [InlineData(".gitattributes")]
    public void GitFiles_ReturnGitIcon(string filename) => Assert.Equal(new Rune(0xF1D3), FileIcons.GetIcon(File(filename)));
}
